# Current Task Tracker — Phase 7C-P1 UI Polish Backlog Triage and Slice Plan (READ-ONLY PLANNING / AWAITING CODEX REVIEW)
...
**✅ FINAL TOUCH UAT FOR 7C-D4-D2 PASSED / ACCEPTED.** CEO confirmed the physical POS keyboard, hardware-scanner, and touch-numpad suite is successful and stable. Tech Lead / CEO authorized the next slice: **Phase 7C-P1 — UI Polish Backlog Triage and Slice Plan (READ-ONLY PLANNING ONLY)**. No implementation, runtime, CSS, or test changes are authorized. Allowed files: `Context.md`, `Task.md`, `docs/reports/phase-7c-p1-ui-polish-backlog-triage.md`. The strategic pivot stands (iOS/iPad Safari focus workarounds DROPPED; hardware-scanner focus deferred to the Native App / Capacitor wrapper phase).

**Phase 7C-P1 — UI Polish Backlog Triage and Slice Plan — READ-ONLY PLANNING / AWAITING CODEX REVIEW — NOT COMMITTED:**

Triage of the Tech Lead / CEO backlog (UI-01…UI-08 + LOGIC-01) into a safety-ordered slice plan. Full analysis in `docs/reports/phase-7c-p1-ui-polish-backlog-triage.md`. Headlines: **LOGIC-01** is the only correctness/data-loss bug — the Product Picker multi-select loops `onProductClick`, which writes a single `uomProduct` state slot, so multiple multi-UOM products overwrite each other (only the last is prompted); fix needs a pending-UOM **queue** and must preserve the F12/Escape modal contracts → its own strictly-reviewed slice, never bundled with UI polish. **UI-08** carries the highest UI risk + an unresolved ambiguity (the "Branch name row" is the global AppShell header, not the POS topbar; and there is no control literally named "Sync" to rename) → CEO clarification required before coding; do last. **UI-04 / UI-03** (default route → `/pos`; POS as a top-level nav item) are the safest, highest-value first wins. **UI-01** (Sales History header radius) must avoid the shared flowbite Table theme (`components/ui/theme.ts`) to not regress Product Picker / report tables. **UI-02** (sidebar toggle) and **UI-03** both edit `AppShell.tsx` → serialize. **UI-05/06** (POS Product Name + Price font adjusters) need a new device-local LocalStorage store + a settings control + clamped sizes (layout/touch-target care); **UI-07** (category modal → list view) is POS-local CSS (modal already has search). **Recommended order:** LOGIC-01 → UI-04 → UI-03 → UI-02 → UI-01 → UI-07 → UI-05/06 → UI-08(blocked). Every slice = its own commit + mandatory Codex review + keyboard-contract re-gate on POS-touching slices.
...
**Next step (7C-P1):** Codex GPT-5.5 High review of the triage/slice plan before Tech Lead closure / any UI or LOGIC implementation authorization. UI-01 through UI-08 and LOGIC-01 remain in the planning inventory — **no implementation authorized yet**. P1 not closed; P1 not committed.
