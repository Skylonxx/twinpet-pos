# Current Task Tracker — Phase 7C-D4-D2 Discount Numpad Clear UX Refinement (IMPLEMENTATION / AWAITING CODEX REVIEW)
...
**Current clean baseline:** `bc0fcf9 fix(pos): add discount numpad clear action` (7C-D4-D1, on top of 7C-D4-D `1a239b4` / 7C-E1 `0c7e924`). **7C-C3, 7C-C4, 7C-D2, 7C-D3-A, 7C-D3-B, 7C-D4-A, 7C-D4-C, 7C-D4-C-1..4, 7C-E1, 7C-D4-D, and 7C-D4-D1 are CLOSED / COMMITTED.** 7C-D4-D2 is an implementation slice on top of this baseline and NOT yet committed.

**✅ FINAL TOUCH UAT (after D4-D1) → ONE NARROW UX REFINEMENT.** CEO ran Final Touch UAT on the physical POS terminal after D4-D1 and rejected the in-grid Clear key: it auto-flowed to a new (5th) grid row and read as a layout bug. CEO direction: return the numpad to the clean decimal keypad, and move clearing to a separate footer/action button labelled `ล้างส่วนลด` (avoid ambiguous `ยกเลิก`, which reads as "close without saving"). Addressed in 7C-D4-D2 below. The strategic pivot stands (iOS/iPad Safari focus workarounds DROPPED; hardware-scanner focus deferred to the Native App / Capacitor wrapper phase). No component-level Escape follow-up and no UI polish authorized.

**Phase 7C-D4-D2 — Discount Numpad Clear UX Refinement (Tech Lead / CEO authorized) — IMPLEMENTATION / AWAITING CODEX REVIEW — NOT COMMITTED:**

Replace the D4-D1 in-grid Clear key with a discount-only footer **action** button. **Scope: `NumpadDialog` opt-in clear-action props + the discount caller + a justified `NumpadDialog.css` button class + tests; NO UOM scan change, NO checkout/payment/write-path change, NO iOS focus hack.** Removed: the 13-key `NUMPAD_KEYS_DECIMAL_CLEAR` layout and the `allowClear` prop (the cause of the 5th-row Clear key). Added to `NumpadDialog`: opt-in, backwards-compatible `clearLabel` / `onClear` / `showClearAction` (default off) → render a labelled footer button BELOW the keypad (between the 3×4 grid and Confirm) — never an extra grid key, so the keypad stays a clean 3×4. The bill-discount `<NumpadDialog>` wires `showClearAction={cart.billDiscValue > 0}` (shown only when a discount exists), `clearLabel="ล้างส่วนลด"`, and `onClear` → `cart.setBillDiscValue(0)` + close + `focusSearch()` (reuses the existing bill setter; no new cart write path; matches the dialog action pattern). The qty `<NumpadDialog>` passes none of the new props → unchanged. **Decimal + zero support intact** (decimal layout `NUMPAD_KEYS_DECIMAL` restored as the discount keypad; seed not floored; `parseFloat||0` confirm). **F12 (`hasBlockingModalOpen`) and Escape (`closeTopModalOnEscape`) still include `discNumpadOpen` — unchanged.** NumpadDialog stays touch-only (no `onKeyDown`/`isComposing`/`Escape`).

**CSS note (justified):** one new `.npd-clear` class in `NumpadDialog.css` (NOT a forbidden file) — a secondary, danger-tinted footer button distinct from the primary Confirm. It is a footer action, NOT positioning of an in-grid Clear key; no other CSS touched. No `POSPage.css` / `index.css` / `styles/variables.css` change.

**Test updates (`src/pages/POSPage.keyboard-contract.test.ts`, section G rewritten for D4-D2):** the in-grid Clear / 13-key `NUMPAD_KEYS_DECIMAL_CLEAR` layout and `allowClear` are fully removed (no 5th grid row); the discount caller exposes `clearLabel="ล้างส่วนลด"` + `onClear` only when `showClearAction={cart.billDiscValue > 0}`; `onClear` sets `cart.setBillDiscValue(0)` and closes; the clear action is opt-in (qty instance has none of the discount-only flags); `NumpadDialog` renders the gated footer button between grid and confirm; decimal + zero remain (`NUMPAD_KEYS_DECIMAL`, no-floor seed, `parseFloat||0`); qty integer contract untouched; NumpadDialog stays touch-only; F12/Escape still include `discNumpadOpen`; UOM scan routing (SKU before UOM barcode) unchanged. All prior D4-A/C/D contracts green.

**Forbidden boundaries honored (D4-D2):** no forbidden CSS (`POSPage.css`/`index.css`/`styles/variables.css`) change — only the justified `NumpadDialog.css` `.npd-clear` class; no `PaymentModal`/`lib/*` change; no UOM scan-routing change; no checkout/payment/order-creation/offline/IndexedDB/manual-review change; no cart/discount/UOM/quantity calculation change (clear action only sets bill discount to 0 via the existing setter); no F12/Escape behavior change (only re-asserted in tests); no LAN/emulator config; no Firebase rules/Functions; no Android/Capacitor/`.claude/`; changed files = `Task.md`, `Context.md`, `src/components/pos/NumpadDialog.tsx`, `src/components/pos/NumpadDialog.css`, `src/pages/POSPage.tsx`, `src/pages/POSPage.keyboard-contract.test.ts`; `stash@{0}` untouched (read-only `git stash list` only).

**Next step (D4-D2):** Codex GPT-5.5 High review of the Clear-UX refinement before Tech Lead closure / commit. Do NOT mark D4-D2 closed. Do NOT authorize the next phase. UI-01 through UI-08 and Logic-01 remain paused. Hardware-scanner focus stays deferred to the Native App / Capacitor wrapper phase.
