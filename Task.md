# Current Task Tracker — Phase 7C-L2 Empty-cart Bill Discount Reset (IMPLEMENTATION / AWAITING CODEX REVIEW)
...
**Current clean baseline:** `203f636 fix(pos): queue multi-uom picker selections` (7C-L1, on top of 7C-P1 `d87110c` / 7C-D4-D2 `ca0d5d8` / 7C-D4-D1 `bc0fcf9` / 7C-D4-D `1a239b4` / 7C-E1 `0c7e924`). **7C-C3, 7C-C4, 7C-D2, 7C-D3-A, 7C-D3-B, 7C-D4-A, 7C-D4-C, 7C-D4-C-1..4, 7C-E1, 7C-D4-D, 7C-D4-D1, 7C-D4-D2, 7C-P1, and 7C-L1 are CLOSED / COMMITTED.** 7C-L2 is an implementation slice on top of this baseline and NOT yet committed.

**✅ 7C-L1 PHYSICAL UAT ACCEPTED (with new findings) → 7C-L2 AUTHORIZED.** CEO accepted the L1 multi-UOM queue fix on the physical terminal; UAT surfaced a money-correctness bug: a bill-level discount can remain on an EMPTY cart and leak into the next customer's sale. Tech Lead / CEO authorized **Phase 7C-L2 — Empty-cart Bill Discount Reset (APPROVED Option A)** — High severity / money correctness / state leakage. **L3 (UOM barcode search display alignment) is deferred** and must not be started. UI-01 through UI-08 remain deferred; no visual UI polish, CSS, or checkout/payment/write-path change is authorized.

**Phase 7C-L2 — Empty-cart Bill Discount Reset (Tech Lead / CEO authorized) — IMPLEMENTATION / AWAITING CODEX RE-REVIEW — NOT COMMITTED (Codex round 1 = NEEDS REVISION → revised):**

**Codex round 1 = NEEDS REVISION (money correctness):** the first cut reset only `billDiscValue`, but `clearCart()` resets the full bill-level set (`billDiscValue` + `billDiscPercent` + `feeRate`). So a `feeRate` on a fee-bearing cart, or a stale `billDiscPercent` mode, could still leak into the next sale after the last item was removed. **Revision:** the empty-cart reset now matches `clearCart()` bill-level semantics exactly, via a shared helper.

**Root cause:** in `useCart` (`src/hooks/pos/useCart.ts`) the bill-level state (`billDiscValue` / `billDiscPercent` / `feeRate`) lives alongside the cart lines. `clearCart` already zeroes all three (so new-sale / hold / clear-cart-confirm, which all route through `cart.clearCart()`, were safe), but the **item-removal paths** — `removeLine`, `changeQty` with qty≤0, and `setLineQty(≤0)` — only delete the line. Removing the LAST item therefore left non-zero bill-level state on an empty cart, silently affecting the next customer's bill. **Fix (minimal, central, parity by construction):** a shared `resetBillLevel` `useCallback` (`setBillDiscValue(0)` + `setBillDiscPercent(false)` + `setFeeRate(0)`) is the single source of truth for the bill-level cleanup, used by BOTH a new empty-cart `useEffect` AND `clearCart` (which now delegates to it) — so the two reset paths can never drift apart again. The effect — `if (cartLines.length === 0 && (billDiscValue !== 0 || billDiscPercent || feeRate !== 0)) resetBillLevel()` — sits in the cart-state layer (not per UI handler) so EVERY emptying path is covered in one place; the all-fields guard prevents render loops and never disturbs a fresh empty cart. **Behavior:** removing the last item / clearing / completing a sale (cart→empty) reset discount value + discount mode + fee to defaults; bill-level state set while the cart is already empty is immediately reset; bill-level state on a cart that still has items is untouched; `restoreCart` repopulates lines AND discount/mode/fee together so the cart is never empty during restore (effect does not fire). **No change** to: totals math (`calcCartTotals` called with the same args), checkout/payment/order-creation/write paths, D4-D scan routing, D4-D2 discount numpad UI / `ล้างส่วนลด`, L1 multi-UOM queue, Product Picker / UOM modal UI, offline / manual-review, or CSS.

**Test updates (new `src/hooks/pos/useCart.contract.test.ts`, +8 L2 tests):** source-level `?raw` contract (the hook is stateful and the vitest unit config is node-env with no DOM / no renderHook — same precedent as the POSPage keyboard-contract suite). Asserts: `resetBillLevel` zeroes all three bill-level fields; the empty-cart effect calls `resetBillLevel()` under an all-fields guard with the `[cartLines.length, billDiscValue, billDiscPercent, feeRate, resetBillLevel]` deps (no inline single-field reset); `clearCart` delegates to the SAME helper (parity by construction); `removeLine` only deletes the line (no inline reset of any bill-level field); `restoreCart` still seeds discount/mode/fee from the bill (not clobbered); all three setters remain available for non-empty carts; `calcCartTotals` args unchanged (no totals-semantics change). Full suite green incl. all prior D4/L1 contracts.

**Forbidden boundaries honored (L2):** no CSS change; no UOM search-display / Product Picker / UomModal / `components/pos/*` change; no `PaymentModal`/`App.tsx`/`lib/*` change; no checkout/payment/order-creation/offline/IndexedDB/manual-review change; no totals-calculation change; no D4-D scan-routing / D4-D2 numpad change; no UI-01…UI-08; no route/auth; no Firebase rules/Functions; no LAN/emulator; no Android/Capacitor/`.claude/`; no L3 work. Changed files = `src/hooks/pos/useCart.ts`, `src/hooks/pos/useCart.contract.test.ts`, `Context.md`, `Task.md`; `stash@{0}` untouched (read-only `git stash list` only). Validation: `tsc -b` clean; `vitest run` **558 passed** (31 files); targeted spec **8 passed**.

**Next step (L2):** Codex GPT-5.5 High **re-review** of the full bill-level empty-cart reset before Tech Lead closure / commit. Then CEO re-checks empty-cart discount + fee on the physical terminal. L2 not closed, not committed. L3 (UOM barcode search display) remains deferred; UI-01 through UI-08 remain deferred.
