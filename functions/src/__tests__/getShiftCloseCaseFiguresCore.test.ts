...
describe('11.2 Authorization (core)', () => {
  ...
  test('auth null -> unauthorized', () => {
    expect(Core.decideAuthorization(Core.curateCallableAuth(null), request, true)).toEqual({
      status: 'unauthorized',
    });
  });
  test('auth undefined -> unauthorized', () => {
    expect(Core.decideAuthorization(Core.curateCallableAuth(undefined), request, true)).toEqual({
      status: 'unauthorized',
    });
  });
  test('token without staffId -> unauthorized', () => {
    const c = Core.curateCallableAuth({ token: { role: 'manager', branchIds: [branchId] } });
    expect(Core.decideAuthorization(c, request, true)).toEqual({ status: 'unauthorized' });
  });
  test('staffId empty string still counts as present (mirrors rules != null semantics)', () => {
    const c = claims({ staffId: '' });
    expect(Core.decideAuthorization(c, request, true)).toBeNull();
  });
  ...
  test('freshnessVerified false -> unauthorized even with manager same branch', () => {
    const c = claims({});
    expect(Core.decideAuthorization(c, request, false)).toEqual({ status: 'unauthorized' });
  });
