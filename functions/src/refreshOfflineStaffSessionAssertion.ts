/**
 * refreshOfflineStaffSessionAssertion — SEC-001 Packet D-1A
 *
 * Authenticated PIN-less offline staff session assertion refresh.
 * Requires:
 * - Authenticated session with non-empty trusted staffId claim
 * - Live user record check (active, not deleted, authVersion match, branch match)
 * - Valid SSCP1 challenge possession proof with purpose == REFRESH (2)
...
export type RefreshOfflineStaffSessionResponse =
  | { ok: true; ssa1Base64: string; srf1Base64: string }
  | { ok: false; code: string };

export async function performRefreshOfflineStaffSessionAssertion(
  database: Firestore,
  auth: AuthLike,
  requestData: unknown,
  nowMs: number = Date.now(),
): Promise<RefreshOfflineStaffSessionResponse> {
  ...
  const deviceCheck = validateDeviceForSession(liveDevice, sscp1, SSCP1_PURPOSE_REFRESH, staffId);
  if (!deviceCheck.ok) {
...
export const refreshOfflineStaffSessionAssertion = onCall({ region: FUNCTIONS_REGION }, async (request) => {
  try {
    return await performRefreshOfflineStaffSessionAssertion(db, request.auth as AuthLike, request.data);
  } catch {
