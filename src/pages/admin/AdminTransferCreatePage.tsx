...
} from '../../components/products/ProductPickerDialog';
...
/**
 * HQ / Admin transfer creation. Unlike the POS form (which is locked to the
 * logged-in branch) the admin explicitly picks BOTH the source and destination
 * branch. The backend `confirmBranchTransfer` takes from/to as plain params and
 * the Firestore rules grant admins full cross-branch write access, so no
 * access-control special-casing is needed here.
 */
export default function AdminTransferCreatePage() {
  ...
  // Products + per-branch stock for the SELECTED source branch.
  const { products, loading } = useProductCrud(fromBranchId || null);
  ...
  // Switching the source branch invalidates picked lines — see onChange below.
  ...
  const destBranches = useMemo(
    () => activeBranches.filter((b) => b.id !== fromBranchId),
    [activeBranches, fromBranchId],
  );

  const linesWithQty = useMemo(() => lines.filter((l) => l.transferQty > 0), [lines]);
  ...
  const validateForm = (): boolean => {
    ...
    if (!fromBranchId) {
      setValidationError('กรุณาเลือกสาขาต้นทาง');
      return false;
    }
    if (!toBranchId) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      window.setTimeout(() => navigate('/admin/transfers'), 500);
    } catch (err) {
  ...
  const fromLabel = fromBranchId ? branchName(fromBranchId) : '—';
  const toLabel = toBranchId ? branchName(toBranchId) : '—';
  ...
  return (
    <div className="inv-adj-page">
      <header className="inv-adj-topbar">
        ...
        <span className="inv-adj-title">สร้างการโอนย้าย (HQ)</span>
      ...
      <div className="inv-adj-scroll">
        <div className="inv-adj-card">
          ...
          <div className="inv-adj-form-grid">
            ...
            <div className="inv-adj-field">
              <label htmlFor="adm-tr-from">สาขาต้นทาง</label>
              <select
                id="adm-tr-from"
                value={fromBranchId}
            ...
            <div className="inv-adj-field">
              <label htmlFor="adm-tr-to">สาขาปลายทาง</label>
              <select
                id="adm-tr-to"
                value={toBranchId}
            ...
            <div className="inv-adj-field inv-adj-form-full">
              <label htmlFor="adm-tr-note">หมายเหตุ</label>
              <textarea
                id="adm-tr-note"
        ...
        <div className="inv-adj-card">
          ...
          <div className="inv-adj-table-wrap">
            {!fromBranchId ? (
              <div className="inv-adj-empty">กรุณาเลือกสาขาต้นทางก่อนเลือกสินค้า</div>
            ) : loading ? (
      ...
      <TransferConfirmDialog
        ...
        fromBranchLabel={fromLabel}
        toBranchLabel={toLabel}
        saving={submitting}
