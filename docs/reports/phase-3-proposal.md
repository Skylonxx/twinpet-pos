...
**Why now:** Phase 2 closed the data-security gaps; the next highest risk is *operational* — a wrong-project deploy, an emulator flag leaking to prod, or a rules/functions change shipped without verification.
...
4. **Deploy command safety.** Harden the deploy scripts: explicit `--project`, no `(default)` database, a confirm/dry-run step, and a guard that refuses deploy if emulator envs are set or the wrong project is active. Review `functions` deploy allowlist (`verifyPinLogin,reconcileOrder,retryReconcile`).
5. **Rules/functions deployment checklist.** A written, ordered checklist: run `test:rules` (must be green) + `functions` unit tests before any `firebase deploy --only firestore:rules` / `--only functions`; verify indexes; never deploy rules without a green rules suite.
