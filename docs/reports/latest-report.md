...
> **Current state:** **Phase 7B-H7-G — Manual Review Ops Durable Rejection Panel** — **IMPLEMENTED / AWAITING CODEX REVIEW — NOT COMMITTED, NOT CLOSED** (Gemini / Tech Lead / CEO — Option A APPROVED; Claude Opus 4.8, High reasoning). Surfaces the durable rejection logs (H7-A model, H7-C store, written by H7-E/H7-F) in the existing Manual Review Ops page as a LOCAL READ-ONLY forensic panel — the Admin/Ops surfacing slice. `src/pages/ManualReviewOpsPage.tsx` adds a section titled `บันทึกการปฏิเสธหลักฐาน (อุปกรณ์นี้)` BELOW the existing manual-review queue. It is gated by the SAME page-level Manager/Admin gate (`canViewManualReviewOps` early-returns the not-authorized Alert before any panel renders), reuses the EXISTING memoized `createIndexedDbReversalStore`, and reads ONLY via `listReversalRejections(store)` (newest-first) through an independent `refreshRejections` loader also gated on `canResolve` (clears data for non-authorized). Read-only columns: เวลา (`r.createdAt`), ประเภท (`r.sourceType` badge), เอกสาร (`r.sourceId`), สาขา (`r.branchId`), รหัส (`r.evidenceCode`), เหตุผล (`r.evidenceMessage`), ผู้ทำรายการ (`r.staffId ?? '—'`); row key is composite `sourceType:sourceId:createdAt` (not `recordId`). An `info` Alert disclaimer states the records are local-device forensic logs only — not server-synced, not a central audit log, not requiring resolution (`...เฉพาะเครื่องนี้เท่านั้น ไม่ได้ซิงก์ขึ้นเซิร์ฟเวอร์ ไม่ใช่ audit log กลาง และไม่ต้องปิดงานจากรายการนี้`); loading/error/empty states match the queue pattern. **Visibility-only:** NO resolve/delete/retry/sync/export/queue/stock/server action button is added (`<Button` count unchanged at 3 = queue resolve + 2 modal); `recordId`/hashes/`serializeReversalRejectionRecord`/raw payloads/qty-cost/`observedDocumentUpdatedAt` are NOT exposed; NO write API (`recordEvidenceRejection`/`recordReversalRejection`) is imported or called in the page. The existing manual-review queue load/resolve/toast/gate/route behavior is unchanged (`listQueue`/`resolveManualReview`/`buildManualReviewResolvePayload` intact). **No catch-site change, no helper/model/log/store/schema change (`recordEvidenceRejection.ts`/`reversalRejectionRecord.ts`/`reversalRejectionLog.ts`/`reversalLocalStore.ts` untouched; no `DB_VERSION` bump; no index), no offline display-label helper, no new route/nav, no separate Admin page, no server resolver/Firestore-rules change, no validation/fail-closed change, no transfer/receiving write-path change, no server-sync/central-audit architecture.** Tests: `src/lib/pos/offline/manualReviewOps.test.ts` (+11 → 21) — a store-level read-path pair (record via `recordEvidenceRejection` → read back newest-first; empty store → empty list) + 9 source-level `?raw` page assertions (per the H6-D2/H7-E precedent). Focused `manualReviewOps` 21 passed; `reversalRejectionLog`/`recordEvidenceRejection`/`reversalLocalStore`/`offlineReversalQueue` 75 passed; **489 web (29 files)** (was 478; +11), `tsc -b` clean, 43 functions resolveReversal (server unchanged). Forbidden-area diff EMPTY; 2 code files changed; `stash@{0}` untouched. **H7-G is NOT closed until Codex GPT-5.5 High + Tech Lead approval; no commit made.**
>
