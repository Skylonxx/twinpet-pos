...
> **Current state:** **Phase 7C-A — Regression Stabilization Pass + Phase 7C-D1 — Operator Runbook** — **IMPLEMENTED / AWAITING CODEX REVIEW — NOT COMMITTED, NOT CLOSED** (Gemini / Tech Lead / CEO — Option B APPROVED; bundled, test/docs-only, no production behavior change). Phase 7C opens the Stabilization & UI Polish phase with a test/docs-only slice. **7C-A** hardens the H7-G `?raw` source-level assertions in `src/lib/pos/offline/manualReviewOps.test.ts` (21 → 24 tests, no production code touched): the brittle whole-page `expect(source.split('<Button').length - 1).toBe(3)` count is replaced by two intent-based tests — a region-scoped check (`rejectionPanelRegion` slices the page from the panel marker comment to the resolve `<Modal>`) asserting the panel contains NO `<Button` and NO action wiring (`onClick`/`openResolve`/`submitResolve`/`resolveManualReview`), plus a positive check that the existing manual-review QUEUE retains its resolve affordance (`onClick={() => openResolve(it)}` + `void submitResolve()`) so stabilization can't silently drop it; the spacing-sensitive `recordId` negatives (`>{r.recordId}`/`title={r.recordId}`) are replaced by a robust single-use check (`r.recordId` appears EXACTLY ONCE and that use is `key={r.recordId}`), keeping hash/serialized internals (`serializeReversalRejectionRecord`/`observedDocumentUpdatedAt`) asserted absent; and a `GUARD` test + comment documents the in-memory store concurrent-write artifact (the in-memory double commits a readwrite txn by REPLACING the whole store map, so concurrently-started txns lose distinct-key writes — last-commit-wins — a property of the TEST DOUBLE only; real IndexedDB serializes readwrite txns per store, so production is unaffected; the guard pins the safe serialized contract the newest-first read-path test relies on). All other safety assertions (gate reuse, loader gating, read-only via `listReversalRejections`, no write API, Thai disclaimer, queue path intact) are preserved — coverage preserved/strengthened (net +3 tests). **7C-D1** adds `docs/operator-runbook-durable-rejection-panel.md`, a plain-Thai operator guide (what the panel is; Manager/Admin-only Manual Review Ops location; what a row means; local-device-only meaning; what it is NOT — not central audit / not server-synced / not stock truth / not a queue item needing resolution; what to do / not do; local-storage-clearing limitation). **No production source change (`ManualReviewOpsPage.tsx` and all offline/store/catch-site files untouched — forbidden-area diff EMPTY), no store/schema/`DB_VERSION` change, no UI behavior/layout/polish change, no POS cashier UX / cart / checkout / stock-mutation change, no server resolver / Firestore-rules change, no route/nav change.** Focused `manualReviewOps` 24 passed; `recordEvidenceRejection`/`reversalRejectionLog`/`reversalLocalStore`/`offlineReversalQueue` 75 passed; **492 web (29 files)** (was 489; +3), `tsc -b` clean, 43 functions resolveReversal (server unchanged). `git diff --stat` = 1 modified test file + 1 new docs file; `stash@{0}` untouched. **7C-A + 7C-D1 are NOT closed until Codex GPT-5.5 High + Tech Lead approval; no commit made. UI polish and POS cashier UX remain future separately-authorized slices.**
>
