...
**Current clean baseline:** `e754461 docs add pos keyboard focus audit` (7C-D3-B audit; 7C-D3-A polish `71d5dac` below). 7C-D4-A (POS keyboard contract tests, test-only) is on top and NOT yet committed.
...
- **7C-D4-A** — POS Keyboard Contract Tests (test-only; **AWAITING CODEX RE-REVIEW — NOT COMMITTED**; revised after Codex NEEDS REVISION): new `src/pages/POSPage.keyboard-contract.test.ts` (**22 tests**, source-level `?raw` per the H7-G precedent — vitest unit env is `node`/no-DOM, POSPage harness makes mounting unsafe) LOCKS the current contracts before any change: scan match/miss (`findByScanCode` priority/trim/empty-guard, Enter-only, clear+focus on match, toast-only on miss) incl. the **locked no-IME-guard GAP**; F12 ↔ checkout parity (single cleaned-up keydown listener, `preventDefault`, `cart>0 && activeShift` = complement of the `disabled` gate); focus-return present (new-sale/customer/payment-close **+ hold + restore + clear-cart + cancel-parked**) + **locked UOM/ItemDiscount focus-return GAPS and the NumpadDialog no-focus-return GAP**; payment confirm = guarded **Red** native button — keyboard-activatable when focused (that is why it is Red), assertion only proves no EXTRA modal-level key handler; NumpadDialog touch-only. No runtime POS change; no helper extraction. `vitest run` 514 passed (+22); `tsc -b` clean.

**Next step:** Codex GPT-5.5 High RE-review of the revised 7C-D4-A test package before Tech Lead closure / commit. Then Tech Lead decision on D4-B (focus-ring polish) / D4-C (IME-and-focus fix planning). Interaction-level POS behavior changes remain separately authorized.
