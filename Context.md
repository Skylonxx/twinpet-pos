...
**Current clean baseline:** `dc8a08b docs: add modal focus ime fix plan` (7C-D4-C; 7C-D4-A `74371db` below). 7C-D4-C-1 (POS IME/composition guard, implementation) is on top and NOT yet committed.
...
- **7C-D4-C-1** — POS IME/Composition Guard (implementation; **AWAITING CODEX REVIEW — NOT COMMITTED**): first slice off the D4-C plan. Added a synchronous early-return in `handleSearchKeyDown` — `if (e.nativeEvent.isComposing || e.nativeEvent.keyCode === 229) return;` after the `Enter` check — so a Thai-IME commit `Enter` no longer fires `findByScanCode`/`addToCart`/clear/miss-toast. Scanner speed preserved (scanners emit no composition events); **no debounce/timers/delay/new listener**; `findByScanCode` unchanged. Flipped the D4-A "no IME guard" GAP test to present-and-ordered-before-scan + added a no-timer/no-debounce assertion; all other contracts unchanged. Targeted 23 passed; `vitest run` 515 passed; `tsc -b` clean. Files: `src/pages/POSPage.tsx`, `src/pages/POSPage.keyboard-contract.test.ts`, trackers.

**Next step:** Codex GPT-5.5 High review of the 7C-D4-C-1 IME guard before Tech Lead closure / commit. Then later slices D4-C-2 (focus-return), D4-C-3 (F12 suppression), D4-C-4 (Escape) — each separately authorized + Codex-reviewed + keyboard-UAT. D4-B (focus-ring polish) remains a separate future option.
