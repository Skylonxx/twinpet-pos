...
**Current clean baseline:** `97a6bb2 fix(pos): guard scan input during IME composition` (7C-D4-C-1; 7C-D4-C `dc8a08b` below). 7C-D4-C-2 (POS focus-return consistency, implementation) is on top and NOT yet committed.
...
- **7C-D4-C-2** — POS Focus-return Consistency (implementation; **AWAITING CODEX REVIEW — NOT COMMITTED**): second slice off the D4-C plan. Restores focus to `#pos-search` after the four audited focus-drop modals via the existing `focusSearch()` helper — UomModal (onSelect+onClose), ItemDiscountModal (onClose, which funnels save+cancel), NumpadDialog (onClose + onConfirm after a successful `setLineQty`), and the category overlay (new `closeCatModal` helper backing all four close routes; `setActiveCategory` left in place). Tests-first: flipped the three D4-A `CURRENT GAP` locks (UOM/ItemDiscount/Numpad) to presence + added a category focus-return test; all other contracts unchanged. **No cart/discount/UOM/quantity/category-filter calc change; NumpadDialog stays touch-only; no F12/Escape/IME/scanner/PaymentModal/CSS change.** Targeted 24 passed; `vitest run` 516 passed; `tsc -b` clean. Files: `src/pages/POSPage.tsx`, `src/pages/POSPage.keyboard-contract.test.ts`, trackers.

**Next step:** Codex GPT-5.5 High review of the 7C-D4-C-2 focus-return implementation before Tech Lead closure / commit. Then later slices D4-C-3 (F12 suppression), D4-C-4 (Escape) — each separately authorized + Codex-reviewed + keyboard-UAT. D4-B (focus-ring polish) remains a separate future option.
