...
**Current clean baseline:** `71d5dac style(pos): improve green-zone cashier readability` (7C-D3-A polish; 7C-D2 audit `ba850e8` below). 7C-D3-B (POS keyboard/focus audit, read-only) is on top and NOT yet committed.
...
- **7C-D3-B** — POS Keyboard UX / Focus Audit (read-only/test-only; **AWAITING CODEX REVIEW — NOT COMMITTED**): `docs/reports/phase-7c-d3-b-pos-keyboard-focus-audit.md` maps focus bootstrap, the Enter-scan contract, the single global F12 listener (gated; active during modals), the absence of global Escape, per-modal Enter/autoFocus, inconsistent focus-return after UOM/Numpad/ItemDiscount close, and a **missing IME/composition guard** on the scan input. No source/behavior change; no test files added. Recommends D4-A (keyboard contract tests), D4-B (focus-ring/label CSS), D4-C (IME/focus-return/Escape/F12-during-modal — read-only planning, separate authorization).

**Next step:** Codex GPT-5.5 High review of 7C-D3-B; then Tech Lead decision on D4-* (keyboard contract tests / focus-ring polish / IME-and-focus fix planning). Interaction-level POS behavior changes remain separately authorized.
