...
**Current clean baseline:** `0c0a624 docs: mark keyboard UX suite ready for manual UAT` (the UAT-pause tracker sync sitting on top of 7C-D4-C-4 implementation `6f0f4c7 fix(pos): close POS modals with Escape`; 7C-D4-C-3 `e6c0ce7`, 7C-D4-C-2 `d4261f5`, 7C-D4-C-1 `97a6bb2` below). **The 7C-D4-C keyboard UX suite (D4-C-1 IME guard → D4-C-2 focus-return → D4-C-3 F12 suppression → D4-C-4 Escape) is COMPLETE.** Manual UAT is **PAUSED** due to a local test-infrastructure blocker: the physical POS terminal on the LAN couldn't reach the dev machine's Vite server / Firebase emulators. **Phase 7C-E1 (Dynamic LAN Test Environment Setup) is ACTIVE / IMPLEMENTATION (config/infrastructure only, NOT committed)** to unblock it. After E1 closes/commits, CEO resumes D4-C Manual UAT at the physical POS terminal (real Thai keyboard + real barcode scanner). No new POS implementation is authorized.
...
- **7C-E1** — Dynamic LAN Test Environment Setup (config/infrastructure only; **ACTIVE / IMPLEMENTATION — NOT COMMITTED**): unblocks D4-C Manual UAT by letting the physical POS terminal on the LAN reach the dev machine's Vite server + Firebase emulators via the serving machine's LAN IP. Three config changes + one shared-helper consumer: (1) `package.json` — new `"dev:lan": "vite --host"` script (existing `dev`/`dev:web`/`dev:emulator` untouched); (2) `firebase.json` — `"host": "0.0.0.0"` added to the `auth`/`functions`/`firestore`/`storage`/`ui` emulators (all existing ports preserved); (3) `src/lib/firebase.ts` — new exported `getEmulatorHost()` (browser → `window.location.hostname`, else `'localhost'`) replacing the hardcoded `'127.0.0.1'` for Firestore/Auth/Storage emulator connections, consulted ONLY inside the existing `USE_EMULATOR` (`import.meta.env.DEV && VITE_USE_EMULATOR==='true'`) guard; (4) `src/lib/auth/verifyPinLogin.ts` — the Functions-emulator connection now consumes the same exported `getEmulatorHost()` (one config line, inside the existing `USE_EMULATOR` guard) so LAN PIN login works (no duplicate host logic). **Production cloud behavior unchanged** (emulator host is never read off the `USE_EMULATOR` path); `localhost`/`127.0.0.1` browsers behave exactly as before. No POS UI / cashier / checkout / payment / cart / UOM / discount / quantity / offline / manual-review / CSS / rules / Functions-source / Android / `.claude` change; no dependency added. `tsc -b` clean.

**Next step — MANUAL UAT PAUSE (LAN unblock in progress):** the 7C-D4-C keyboard UX suite (D4-C-1..4) is COMPLETE and COMMITTED; implementation is PAUSED. CEO / Tech Lead will conduct Manual UAT at the physical POS terminal with a real Thai keyboard and real barcode scanner (IME guard, scanner speed/no-latency, focus-return after UOM/ItemDiscount/Numpad/category, F12 inert while a blocking modal is open, Escape close/cancel/dismiss only, and Escape never triggering a Red action). **No new implementation is authorized.** The component-level Escape pass for OpenShift / CloseShift / CashTransaction (whose internal submit/Z-report state is not observable from the page) is DEFERRED pending UAT findings + separate authorization. D4-B (focus-ring polish) remains a separate future option; no UI polish is authorized.
