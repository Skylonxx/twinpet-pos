...
**Current clean baseline:** `c875e9a docs: sync manual review ops rejection panel tracker after closure` (H7-G doc-sync; H7-G impl `86a628e` directly below). Phase 7C-A + 7C-D1 are implemented on top and NOT yet committed.
...
- **7C-A** — Regression Stabilization Pass (**IMPLEMENTED / AWAITING CODEX REVIEW — NOT COMMITTED, NOT CLOSED**; Option B APPROVED): test/docs-only hardening of the H7-G `?raw` source-level assertions in `src/lib/pos/offline/manualReviewOps.test.ts` — replaced the brittle whole-page `<Button` count with region-scoped intent checks (panel has no button/action wiring; queue retains its resolve affordance), replaced spacing-sensitive `recordId` negatives with a robust "used exactly once, only as the React key" check, and added a GUARD test + comment documenting the in-memory store concurrent-write artifact (whole-map-replace commit → concurrent distinct-key writes are last-commit-wins in the test double only; real IndexedDB serializes readwrite txns; production unaffected). Safety coverage preserved/strengthened (net +3 tests, 489 → 492 web). **No production code touched; no `ManualReviewOpsPage.tsx` diff; no store/schema/`DB_VERSION`/server/rules/write-path/UI-behavior change.**
- **7C-D1** — Operator Runbook for Durable Rejection Panel (**IMPLEMENTED / AWAITING CODEX REVIEW — NOT COMMITTED, NOT CLOSED**): new `docs/operator-runbook-durable-rejection-panel.md`, plain-Thai operator guide explaining the panel, its Manager/Admin-only Manual Review Ops location, the local-device-only limitation, what it is NOT (not central audit / not server-synced / not stock truth / not a queue item needing resolution), what operators should do, what not to do, and the local-storage-clearing limitation.

**Next step:** Codex GPT-5.5 High review of 7C-A + 7C-D1, then Tech Lead approval → doc-sync/commit. No UI polish or POS cashier UX work has started; those remain future separately-authorized slices.
