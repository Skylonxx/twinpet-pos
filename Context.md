...
**Current clean baseline:** `74371db test(pos): add keyboard contract coverage` (7C-D4-A; 7C-D3-B audit `e754461` below). 7C-D4-C (Modal Focus & IME Fix Planning, read-only) is on top and NOT yet committed.
...
- **7C-D4-C** — Modal Focus & IME Fix Planning (read-only/planning; **AWAITING CODEX REVIEW — NOT COMMITTED**): `docs/reports/phase-7c-d4-c-modal-focus-ime-fix-plan.md` blueprints the four D3-B follow-ups — IME/composition guard on `handleSearchKeyDown` (scanner-safe), F12 modal suppression (stop PaymentModal stacking), focus-return consistency (UOM/ItemDiscount/Numpad/category), and a per-modal **Escape matrix** (corrects "Escape globally absent": CustomerPicker + DestructiveConfirm already have it; rule = Escape may only cancel, never confirm a Red action). Verified PaymentModal has NO backdrop dismissal (X-only), correcting D3-B prose. Recommends ordered tests-first slices D4-C-1..4, each separately authorized + Codex-reviewed + keyboard-UAT. No implementation; no test/runtime change.

**Next step:** Codex GPT-5.5 High review of the 7C-D4-C planning report before Tech Lead closure or ANY D4-C implementation authorization. D4-B (focus-ring polish) remains a separate future option. Interaction-level POS behavior changes remain separately authorized.
