...
- **Phase 7B-H6-F1 — Transfer Reversal Evidence Rejection Visibility** (UI display-only): a pure exhaustive `getTransferReversalEvidenceMessage(code)` maps each `TransferReversalEvidenceCode` → friendly Thai message (unknown code → generic fallback); the two transfer cancel pages' catch blocks surface that message + raw code when a `TransferReversalEvidenceError` is caught (non-evidence errors keep the existing fallback); Manual Review Ops shows a read-only `evidenceSource` label column via a **page-local** `getEvidenceSourceLabel`. **No validation/fail-closed/offline-queue/IndexedDB/`src/lib/pos/offline`-runtime/server-resolver/transfer-write-path change** — the thrown error (type, code, message) is unchanged. (Codex blocker fix: the label helper was moved out of `src/lib/pos/offline/manualReviewOps.ts` into the page; both offline files have no diff.) No durable rejection log (future separately-authorized slice). 410 web tests (25 files), `tsc -b` clean, `functions resolveReversal` 43 unchanged.
