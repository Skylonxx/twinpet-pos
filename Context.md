...
**Current clean baseline:** `d4261f5 fix(pos): restore search focus after POS dialogs` (7C-D4-C-2; 7C-D4-C-1 `97a6bb2` below). 7C-D4-C-3 (POS F12 modal suppression, implementation) is on top and NOT yet committed.
...
- **7C-D4-C-3** — POS F12 Modal Suppression (implementation; **AWAITING CODEX REVIEW — NOT COMMITTED**): third slice off the D4-C plan. New derived `hasBlockingModalOpen = Boolean(uomProduct || pickerOpen || discountLineKey || qtyNumpadLineKey || showCloseShift || holdNoteOpen || suspendedListOpen || showCashTx || catModalOpen || isSortingModalOpen || confirmModalState.open || checkout.customerModalOpen)`; the F12 `onKey` does `preventDefault()` then `if (hasBlockingModalOpen) return;` BEFORE the unchanged `cart>0 && activeShift` open-gate; effect deps += `hasBlockingModalOpen`. OpenShift excluded (F12 already needs `activeShift`). Tests-first: added a suppression test (predicate enumerates the modal states; suppression precedes `setPaymentOpen(true)`; `preventDefault` precedes it; deps include the predicate) + a no-new-listener/no-F12-in-PaymentModal test; all prior F12 parity + checkout-disabled-complement tests preserved. **No checkout/payment/processing/write-path/cart/UOM/discount/quantity change; no Escape/IME/focus-return change; single listener + cleanup unchanged; no PaymentModal/CSS change.** Targeted 26 passed; `vitest run` 518 passed; `tsc -b` clean. Files: `src/pages/POSPage.tsx`, `src/pages/POSPage.keyboard-contract.test.ts`, trackers.

**Next step:** Codex GPT-5.5 High review of the 7C-D4-C-3 F12 suppression before Tech Lead closure / commit. Then the final slice D4-C-4 (Escape) — separately authorized + Codex-reviewed + keyboard-UAT. D4-B (focus-ring polish) remains a separate future option.
